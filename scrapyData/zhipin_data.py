...
def getdata(filed):
    sql = "select %s from 产品" % filed
    cursor.execute(sql)
    # 返回查询到的所有记录(元组类型)
    data = cursor.fetchall()
    # 统计相同字段值出现的次数
    count = dict(Counter(each[0] for each in data))     
    # 画图的数据为列表形式
...
# district_list,district_count = getdata('city')                # 城市
positionName_list,positionName_count = getdata('positionName')  # 职位
workYear_list,workYear_count = getdata('seniority')             # 工作年限
...
# 柱状
bar = Bar('工作年限','')
bar.add('',workYear_list,workYear_count)
bar.show_config()
bar.render('seniority.html')

# bar = Bar('职位','')
# bar.add('',positionName_list,positionName_count)
# bar.show_config()
# bar.render('positonName.html')
